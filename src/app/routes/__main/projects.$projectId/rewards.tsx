...
import { RewardsView } from "@app/ui/main/project/rewards";
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  const title = "Izzy's Independence Board - Rewards";
  const description =
    "Spend your earned points on rewards! From extra tablet time to sleepovers.";
  const image = "https://jira-clone.fly.dev/static/images/readme/project.png";
  const url = `https://jira-clone.fly.dev/projects/${project.id}/rewards`;
...
export function ErrorBoundary({ error }: { error: Error }) {
  console.error(error);
  const errorMessage = "The rewards page failed. Navigate to the board page";
...
export default function RewardsRoute() {
  ...
  return <RewardsView project={project} rewards={rewardsMock} />;
}
